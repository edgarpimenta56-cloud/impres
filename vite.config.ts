...
// Load site config with fallback so the project builds outside the Figma Make environment
let siteConfiguration: FigmaSiteConfiguration = {
  title: 'Arte e Cultura — Sistema de Empréstimos',
  description: 'Sistema de controle de empréstimos de equipamentos culturais.',
  language: 'pt-BR',
  robots: { index: true },
  accessibility: { addBypassLinks: true },
}
try {
  const raw = fs.readFileSync(path.resolve(__dirname, '.figma/make/site.json'), 'utf-8')
  siteConfiguration = JSON.parse(raw)
} catch { /* running outside Figma Make — use defaults above */ }
...
export default defineConfig(({ mode }) => {
  ...
  // BASE_PATH env var supports GitHub Pages subdirectory deploys (e.g. /arte-e-cultura/).
  // FIGMA_PUBLIC_URL is used by the Figma Make preview infrastructure.
  const base =
